...
static VideoConfig *video_config_create(const char *uri) {
    ...
    LOGD("video_config_create, fullFeatured: %d, fd: %d, codecName: %s",
            videoConfig->fullFeatured,
            videoConfig->fileDescriptor,
            videoConfig->avVideoCodec->long_name);
    return videoConfig;
}
